<div{{ htmlAttributes($containerId ? ['id' => $containerId] : null) }}{{ classTag($type . '-' . Str::slug($name) . '-container', 'switch', 'custom-control', $containerClasses) }}{{ htmlAttributes($containerHtmlAttributes) }}>
    <input id="{{ $componentId }}"{{ classTag($type . '-' . Str::slug($name) . '-component', 'custom-control-input', $componentClasses, $validationClass) }} type="checkbox" name="{{ $name }}"{{ htmlAttributes($componentHtmlAttributes) }}>
